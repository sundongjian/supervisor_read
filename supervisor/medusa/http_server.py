...
#                                                All Rights Reserved.
#
# $Id: filename revisionnumber date time author state locker $  版本管理
...
try:
    # unquote用于url的转码（防止正文中出现/等）
    # splitquery 用于对url进行键值对拆分
...
class http_request:
    ...
    def __init__ (self, *args):
        ...
        '''
        (<supervisor.http.deferring_http_channel connected '' at 0x7fa350b44560 channel#: 1 requests:0>,
        u'POST /RPC2 HTTP/1.1',
        u'POST',
        u'/RPC2',
        u'1.1',
        [u'Host: localhost',
        u'Accept-Encoding: identity',
        u'Content-Length: 122',
        u'Content-Type: text/xml',
        u'Accept: text/xml',
        u'User-Agent: xmlrpclib.py/1.0.1 (by www.pythonware.com)'])
        :param args:
        '''
    ...
    def __setitem__ (self, key, value):
        # 当使用 a=http_request()  a['test'] = '100' 自动调用
    ...
    def build_reply_header (self):
        # 制造response的头，self.reply_code 一般为200
    ...
    # 标题相同的  # 。
    # 通常你可以把这样的标题折叠在一起，分开
    # 用逗号分隔它们的内容(例如Accept: text / html, text / plain)
    # 但是最大的例外是Set - Cookie头。
    # 字典为中心的。
    #---------------------------------------------------
    ...
    def remove_header(self, name, value=None):
        ...
        # Remove things from the old dict as well
        # 如果value是None或者找到对应的key value ，删除
