...
class Controller(cmd.Cmd):
    ...
    def __init__(self, options, completekey='tab', stdin=None,
        ...
        #   plugin_factories   class DefaultControllerPlugin(ControllerPluginBase)
        for name, factory, kwargs in self.options.plugin_factories:
            plugin = factory(self, **kwargs) #
            # factory  DefaultControllerPlugin类 父类：ControllerPluginBase    初始化参数：本身self  controller
            # print(plugin)
            # print(dir(plugin))
            '''['__doc__', '__init__', '__module__', '_clearresult', '_doc_header', '_formatChanges', '_formatConfigInfo', 
            '_show_statuses', '_signalresult', '_startresult', '_stopresult', '_tailf', 'ctl', 'do_add', 'do_avail', 
            'do_clear', 'do_exit', 'do_fg', 'do_help', 'do_maintail', 'do_open', 'do_pid', 'do_quit', 'do_reload',
             'do_remove', 'do_reread', 'do_restart', 'do_shutdown', 'do_signal', 'do_start', 'do_status', 'do_stop', 
             'do_tail', 'do_update', 'do_version', 'doc_header', 'help_add', 'help_avail', 'help_clear', 'help_exit',
              'help_fg', 'help_maintail', 'help_open', 'help_pid', 'help_quit', 'help_reload', 'help_remove', 'help_reread',
               'help_restart', 'help_shutdown', 'help_signal', 'help_start', 'help_status', 'help_stop', 'help_tail', 
               'help_update', 'help_version', 'listener', 'name']
            '''
            for a in dir(plugin):
                if a.startswith('do_') and callable(getattr(plugin, a)):
                    self.vocab.append(a[3:])
            # getattr(plugin, 'do_testing')('aa')
    ...
    def exec_cmdloop(self, args, options):
        ...
        try:
            ...
            # cmd 负责不断添加行消息到cmdqueue,所有命令到此为止
            self.cmdloop()
            # cmdloop 会自动执行对应的函数，如输入start  st 会执行 do_start(st)
        except KeyboardInterrupt:
            self.output('')
            pass

    # def do_working(self,arg):
    #     self.output('******************************')
...
class DefaultControllerPlugin(ControllerPluginBase):
    ...
    def do_testing(self,arg):
        self.ctl.output('testing................................')
    ...
    def do_start(self, arg):
        ...
        names = arg.split()
        # 得到supervisor，现在可以使用supervisor.startProcess(name)进行远程调用了
        supervisor = self.ctl.get_supervisor()
